...
# triggered when press on save button
def SubmitData():
...
def OnSelected(evebt):
    ...
    # label
    lbl_title = Label(FormTitle, text="Updating INFO", font=('arial', 16), bg="#AEC6CF", width=300)
...
def average():
    global mem_id
    AGE.set("")
    averageWindow = Toplevel()
    averageWindow.title("AVERAGE Age List")
    width = 400
    height = 300
    ...
    y = ((screen_height / 2) + 20) - (height / 2)
    averageWindow.resizable(0, 0)
    averageWindow.geometry("%dx%d+%d+%d" % (width, height, x, y))
    tree = ttk.Treeview(averageWindow, columns=("Average Age"))
    tree.heading("#1", text="Average Age")
    tree.pack()
    conn = sqlite3.connect("infoo.db")
    cursor = conn.cursor()
    cursor.execute("SELECT AVG (Age) FROM member")
    fetch= cursor.fetchall()
    for data in fetch:
        tree.insert('', 'end', values=data)
    conn.commit()
...
def AddNewWindow():
    ...
    # lables
    lbl_title = Label(FormTitle, text="Adding New INFO", font=('arial', 16), bg="#AEC6CF", width=300)
    lbl_title.pack(fill=X)
    lbl_firstname = Label(ContactForm, text="Firstname", font=('arial', 14), bd=5)
    lbl_firstname.grid(row=0, sticky=W)
    lbl_lastname = Label(ContactForm, text="Lastname", font=('arial', 14), bd=5)
    ...
    # button when press on save to call submit function
...
Top.pack(side=TOP)
Mid = Frame(root, width=500, bg="#AEC6CF")
Mid.pack(side=TOP)
MidLeft = Frame(Mid, width=100)
MidLeft.pack(side=LEFT, pady=10)
MidLeftPadding = Frame(Mid, width=370, bg="#AEC6CF")
...
# label of main GUI
lbl_title = Label(Top, text="Information Management  System", font=('arial', 16), width=500)
lbl_title.pack(fill=X)

# button of add or delete
btn_add = Button(MidLeft, text="ADD NEW", bg="white", command=AddNewWindow)
btn_add.pack()
btn_delete = Button(MidRight, text="DELETE", bg="white", command=DeleteData)
btn_delete.pack(side=RIGHT)
btn_new = Button(MidRight, text="average", bg="white", command=average)
